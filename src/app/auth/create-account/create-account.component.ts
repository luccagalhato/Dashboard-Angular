...
export class CreateAccountComponent implements OnInit {
  ...
  async createAccount(event: any) {
    ...
    this.loading = true;
    if (this.form.valid) {
      await this.afAuth.createUserWithEmailAndPassword(this.form.value.email, this.form.value.password).then((userCredential) => {
        if (userCredential.user) {
          userCredential.user.sendEmailVerification({
            url: `${environment.domain}`
          })
          this.sys.showToast('Conta criada com sucesso!', { classname: 'bg-success text-light', delay: 4000 })
